...
void stockea(struct artxSucursal tabla[], unsigned int longiTabla)//lineal, terriblemente lento
{
    ...
    int indice, indexaux;
    int artabuscar;
    int sucursalenproblemas;
    for(indice=0;indice<longiTabla;indice++)
    {
        if(tabla[indice].cantidad==0)
        {
            artabuscar=tabla[indice].idarticulo;
            sucursalenproblemas=tabla[indice].idsucursal;
            for(indexaux=0;indexaux<=longiTabla;indexaux++)
            {
                if(tabla[indexaux].idarticulo==artabuscar && tabla[indexaux].cantidad >= 5)
                {
                    printf("La sucursal %d puede abastecer a la %d en el articulo %d\n",tabla[indexaux].idsucursal,sucursalenproblemas,artabuscar);
                }
            }
    ...
    printf("Tiempo usado %lf",cpu_time_used);
}
int main()
{
    srand(time(NULL));
    ...
    printf("Inserte numero de articulos. No superar los 100 mil. Tenga piedad de su RAM\n");
    ...
    stockea(tablaArtxSucur,numArtxSucur);
    getchar();
    getchar();
    return 0;
}
